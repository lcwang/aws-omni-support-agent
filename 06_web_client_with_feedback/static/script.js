...
let currentQuestion = ''; // 保存当前问题（用于反馈系统）
...
function addMessage(content, role) {
    ...
    // 为 assistant 消息生成唯一 ID 和添加 data 属性
    if (role === 'assistant') {
        const messageId = `msg_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        messageDiv.setAttribute('data-message-id', messageId);
        messageDiv.setAttribute('data-question', currentQuestion);
        messageDiv.setAttribute('data-answer', content || '');  // 允许空内容
        messageDiv.setAttribute('data-interaction-type', 'qa');
        messageDiv.setAttribute('data-retrieval-source', 'unknown');
        messageDiv.setAttribute('data-rag-documents', '[]');
    }
...
async function streamResponse(message, attachmentData = []) {
    ...
    let fullContent = '';
    let retrievalResults = [];  // Store retrieval results for feedback
    ...
    try {
                ...
                if (line.startsWith('data: ')) {
                    try {
                        ...
                        if (data.error) {
                            ...
                            // Capture retrieval results for feedback system
        ...
        // Update message data attributes with complete answer and retrieval results
        const messageContainer = assistantTextDiv.parentElement.parentElement;
        const messageId = messageContainer.getAttribute('data-message-id');
        const contentDiv = assistantTextDiv.parentElement;

        // Update answer with full content
        messageContainer.setAttribute('data-answer', fullContent);

        // Update retrieval results if available
        ...
        // Check if feedback buttons don't already exist
        if (messageId && !contentDiv.querySelector('.feedback-container')) {
            const feedbackDiv = document.createElement('div');
            feedbackDiv.className = 'feedback-container';
            feedbackDiv.setAttribute('data-message-id', messageId);
            feedbackDiv.innerHTML = `
